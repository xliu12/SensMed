...
med_data <- S7::new_class("med_data",
                              ...
                              constructor = function(data, vars, weights, a1, a0) {
                                  ...
                                  S7::new_object(
                                      ...
                                      a1 = a1,
                                      a0 = a0,
                                      data_1 = shift_data(data, vars@A, a1, center = vars@cluster_id),
                                      data_0 = shift_data(data, vars@A, a0, center = vars@cluster_id),
                                      data_indepM = data,
                                      data_0_indepM = data,
                                      data_1_indepM = data
                                      # data_0zp = data.frame(),
...
shift_data <- function(data, trt, shift = 1, center = NA_character_) {
    ...
    for (a in trt) {
        ...
        if (!is.na(center)) {
            ...
            data[[glue("{a}_cwc")]] <- shift[[a]]
            data[[ glue("{a}_clmean") ]] <- shift[[a]]
        }
    }
    data
}
