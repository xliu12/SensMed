...
fit_alpha_each <- function(med, a_c, a_mc, m_ac, m_each, folds, learners_weights, control, covariates_cl = FALSE, benchmark_covariates = NULL) {
    ...
    # med_U <- med
    # med_U@vars@C <- base::union(med@vars@C, benchmark_covariates)
    # long_a_c <- a.c(med_U, folds, learners_weights, control)
    ...
    alpha_each_list <- lapply(m_each, \(m_each1) {
        ...
        ay <- as.numeric(substr(m_each1[["ay"]], 6, 6))
            alpha_each <- data.frame(
                ...
                # long.short_alpha2 = alpha.amc(A=med@data[[med@vars@A]], ay, am, long_a_c, a_mc),
...
# fit treatment and mediator models -----------------
...
alpha.ac <- function(A, am, a_c) {
    ...
    # normalize(ipw_a) # no weight normalization here, so to compute nu2s
    ipw_a
}
...
alpha.amc <- function(A, ay, am, a_c, a_mc, m_ac) {
    ...
    if (!is.null(m_ac)) {
        ...
        h_am <- ( 1*(A==ay) * a_mc[, glue("a({am}|m,c)")] ) / #(bound(a_c[, glue("a({am}|c)")], tol = 1e-2) * a_mc[, glue("a({ay}|m,c)")])
            pmax(a_c[, glue("a({am}|c)")] * a_mc[, glue("a({ay}|m,c)")], 1e-2)
        # bound(a_c[, glue("a({am}|c)")] * a_mc[, glue("a({ay}|m,c)")], tol = 1e-4)
    }


    # normalize(h_am) # no weight normalization here, so to compute nu2s
    h_am
}
