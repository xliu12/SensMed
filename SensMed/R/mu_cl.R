...
estimate_mu_cl <- function(med, m_each, folds, learners_regressions, control, which.M, covariates_cl = FALSE, benchmark_covar = NULL) {
    ...
    for (i in seq_along(mu_folds)) {
        ...
        mu_folds[[i]] <- mu_eachM(train, valid, med@vars, m_each,  learners_regressions, control, which.M, covariates_cl, benchmark_covar)
...
mu_eachM <- function(train, valid, vars, m_each, learners, control, which.M, covariates_cl = FALSE, benchmark_covar = NULL) {
  # vars <- med@vars
  ...
  if (covariates_cl) {
    varnames_1 <- na.omit(c(glue("{c(vars@A, vars@C)}_cwc"), glue("{c(vars@Y, vars@M)}_clmean"), vars@Wj))
    if ( !is.null(which.M) ) {
      # varnames_1 <- na.omit(c(vars@A, vars@C, glue("{c(vars@A, vars@M[-which.M], vars@C)}_clmean"), vars@Wj))
      varnames_2 <- na.omit(c(glue("{c(vars@A, vars@C, vars@M[-which.M])}_cwc"), glue("{c(vars@Y, vars@M[which.M])}_clmean"), vars@Wj))
    }
    ...
    for (i in 1:length(m_each)) {
      ...
      if (m_each1[["az"]] != "") {
        ...
        mu_fit[[3]] <- mu_y$preds$data
        b_list_train[[ 3 ]] <- predict(mu_y, train[[ m_each[[i]]["ay"]  ]])
        b_list_valid[[ 3 ]] <- mu_y$preds[[ m_each[[i]]["ay"]  ]]
        ...
        b_list_train[[2]] <- predict(mu2, train[[ m_each[[i]]["az"] ]])
        b_list_valid[[2]] <- mu2$preds[[ m_each[[i]]["az"] ]]
        mu_fit[[2]] <- mu2$preds$data
        
        
        mu1 <- mlr3superlearner::mlr3superlearner(
          data = data.frame(train$data[, na.omit(varnames_1)], 
                            tmp_pseudo_y = b_list_train[[2]] ),
          target = "tmp_pseudo_y",
          library = learners,
          outcome_type = "continuous",
          folds = control$mlr3superlearner_folds,
        ...
        if ( length(m_each[[i]]) == 3 ) {
          b_list_train[[1]] <- predict(mu1, train[[ m_each[[i]]["am"] ]])
          b_list_valid[[1]] <- mu1$preds[[ m_each[[i]]["am"] ]]
          
        }
        if ( length(m_each[[i]]) == 4 ) {
          b_list_train[[1]] <- predict(mu1, train[[ m_each[[i]]["am.p"] ]]) - predict(mu1, train[[ m_each[[i]]["am.m"] ]])
          b_list_valid[[1]] <- mu1$preds[[ m_each[[i]]["am.p"] ]] - mu1$preds[[ m_each[[i]]["am.m"] ]]
        }
        
        mu_fit[[1]] <- mu1$preds$data
