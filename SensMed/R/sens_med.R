...
#' @param cluster_id (optional) The column name of the identifier for each cluster
#' @param learners_outcome A character vector of \code{mlr3superlearner} methods for estimating the regressions following R. Liu et al. (2024) "General targeted machine learning for modern causal  mediation analysis" and the accompanying R package \code{crumble}.
#' @param cf.y (optional) A \code{numeric} vector of the partial R-squared sensitivity parameters for the regressions, that is, R^2_\{Y-theta_\{2,s\} ~ theta_2 -theta_\{2,s\} \} and R^2_\{bM-theta_\{1,s\} ~ theta_\{1,u,s\} -theta_\{1,s\} \}.
#' @param cf.m (optional) A \code{numeric} vector of the partial R-squared sensitivity parameters for the weights (or, Riesz representers), that is, 1-R^2_\{alpha_2 ~ alpha_\{2,s\}\} and 1-R^2_\{alpha_1 ~ alpha_\{1,s\}\}.
...
Sens.Mediate <- function(
    ...
    if (!is.null(benchmark_covariates)) {
      ...
      if (num_M > 1) {
        ...
        Benchmark_res[["IIE_M1"]] <- benchmark.effect(
          med1_cwc,
        ...
        Benchmark_res[["IIE_M2"]] <- benchmark.effect(
          med2_cwc,
    ...
    ## Contour plots -----
    sens_grid <- list()
    contour_plot_file = NULL # draw plot outside the function
    if (!is.null(contour_plot_file)) {
      ...
      if (num_M > 1) {
        ...
        bench.cf.y <- Benchmark_res[["IIE_M1"]][[1]]$Gain_mu 
        bench.cf.m <- Benchmark_res[["IIE_M1"]][[1]]$Gain_alpha
        add_benchmark_max <- sens_IIE_Mjo(med1_cwc, mus_IIE_M1, alphas_each_IIE_M1_r, m_each_IIE_M1, params = "IIE_M1", 
        ...
        add_benchmark_min <- sens_IIE_Mjo(med1_cwc, mus_IIE_M1, alphas_each_IIE_M1_r, m_each_IIE_M1, params = "IIE_M1", 
        ...
        # contour_plot_file = "Example/Results"
        
        pdf(file = as.character(glue("{contour_plot_file}/contour_IIE_M1.pdf")), width = 9, height = 9)
        ...
        pdf(file = as.character(glue("{contour_plot_file}/contour_IIE_M2.pdf")), width = 9, height = 9)
      ...
      # contour_plot_file = "Example/Results_NoCluster"
      pdf(file = as.character(glue("{contour_plot_file}/contour_IDE.pdf")), width = 9, height = 9)
      ...
      # contour_plot_file = "Example/Results_NoCluster"
      pdf(file = as.character(glue("{contour_plot_file}/contour_IIE_Mjo.pdf")), width = 9, height = 9)
    ...
    # output ----------------
    med_sens <- list(estimates = estimates
                     , rv_res = rv_res
                     , sens_fit_res = sens_fit_res
                     , Benchmark_res = Benchmark_res
                     )
    # med_sens <- mget(ls(envir = environment()))
